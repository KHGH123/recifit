...
recipe_search_agent = Agent(
    ...
    instruction="""
        - 호출한 쪽(root_agent)이 음식명과 함께 걸러야 할 단어 목록(알레르기/
          제외 재료, 취향 기반 회피 키워드)을 알려주면, search_recipes를
          호출할 때 그 단어들을 exclude_terms 인자로 그대로 넘긴다 — 직접
          판단해서 후보를 빼지 말고, 이 필터링은 search_recipes 안에서
          코드로 처리된다.
...
root_agent = Agent(
    ...
    instruction="""
        ...
        recipe_id는 대화 전체에서 도구 호출 인자로만 쓴다. 딱 하나
        예외는 4번에서 후보 목록을 보여줄 때 각 레시피 이름 바로 뒤에
        붙이는 `[[recipe_id]]` 마커뿐이다(화면이 이 마커를 읽고 지워서
        사용자에게는 실제로 보이지 않는다). 그 외에는 목록 설명, 상세
        설명, 질문, 사과·재시도 메시지 등 어디에도 recipe_id나
        "레시피 ID" 같은 표현을 절대 넣지 않는다 — 검색이 실패해서
        다시 검색하거나 스스로 실수를 인정할 때도 이 규칙은 그대로
        적용된다.
        ...
        1. 사용자 요청에서 다음 조건을 추출한다: 원하는 음식/재료, 예산(원),
           ...
           위 항목과 별도로, "오늘은 매운 거 싫어", "느끼한 거 별로",
           "가벼운 거 먹고 싶어"처럼 알레르기는 아니지만 그날의 취향/기분을
           말한 경우, 그걸 실제 재료 원문에서 걸러낼 수 있는 구체적인
           한국어 키워드로 스스로 변환해 avoid_keywords 목록을 만든다
           (예: "매운 거 싫어" → ["매운", "청양고추", "고추장", "불닭",
           "얼큰"]). avoid_keywords는 excluded_items(알레르기/제외 재료)와
           다르다 — 다음 단계에서 후보 레시피를 거를 때만 쓰고,
           7번(장보기 목록에 넣을 재료를 추리는 단계)에서는 쓰지 않는다.
           사용자가 이미 그 취향을 감안해 고른 레시피의 실제 재료를 다시
           빼면 안 되기 때문이다. 해당하는 말이 없었으면 빈 목록으로 둔다.
        ...
        2. recipe_search_agent를 (구체적인 메뉴 이름마다, 필요하면 여러 번)
           호출해 후보 레시피를 모은다 (최대 5개 정도로 추린다). 호출할
           때마다 1번에서 만든 excluded_items와 avoid_keywords를 합친
           목록을 같이 전달해서, recipe_search_agent가 search_recipes의
           exclude_terms로 그대로 넘기게 한다 — 알레르기 재료나 그날
           기피하는 취향(예: 매운맛)이 들어간 레시피는 이 단계에서 코드로
           걸러지고 나서야 후보로 넘어온다. 각 후보의 recipe_id, 이름,
           재료 원문, 인분 원문을 기억해둔다. recipe_id는
        ...
        4. 사용자에게 후보 레시피 목록을 아래 형식으로 보여준다:
           ...
           마침표, 굵게 표시한 레시피 이름, 그 이름 뒤에 공백 없이
           바로 `[[recipe_id]]` 형식으로 그 후보의 recipe_id를 대괄호
           두 겹으로 감싸 붙인다(예: "1. **김치찌개**[[6746239]]") —
           2번에서 기억해둔 그 후보의 recipe_id를 한 글자도 바꾸지 않고
           그대로 쓴다. 화면(웹 프론트엔드)이 이 마커를 읽어서 번호와
           recipe_id를 정확히 연결하고 마커 자체는 지운 뒤 보여주므로,
           사용자 눈에는 레시피 이름만 보인다. 그 아래 줄바꿈해서
           하이픈으로 시작하는 간단한 설명 한 줄, 또 하이픈으로 시작하는
           "예상 가격: 약 OOOO원" 줄.
           ...
           recipe_id/레시피 ID는 위 `[[recipe_id]]` 마커 자리를 제외하면
           이 목록을 포함해 사용자에게 보이는 어떤 문구에도 절대 나오면
           안 된다 — 검색이 실패해서 다시 시도하거나 스스로 실수를
           인정하는 메시지를 쓸 때도 마찬가지다. "레시피 ID" 같은 표현
           자체도 절대 넣지 않는다.
