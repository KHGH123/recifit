...
root_agent = Agent(
    ...
    instruction="""
        ...
        4. 사용자에게 후보 레시피 목록을 아래 형식으로 보여준다:
           ...
           한 줄로 충분하다.) 목록 마지막에 어떤 걸 선택할지 물어보고
        ...
        5. get_recipe를 recipe_id로 호출해서 조리순서·이미지를 가져온다.
           ...
           에러가 아니면 instructions 배열의 내용을 그대로(내용을 더하거나
           빼거나 바꾸지 않고) 보여주되, 읽기 편하게 다듬어서 보여준다:
           원문이 여러 동작을 쉼표나 띄어쓰기만으로 한 문장에 몰아넣은
           경우, 의미를 바꾸지 않는 선에서 문장을 끊어 번호를 세분화하거나
           마침표·띄어쓰기를 정리한다. 재료명·수량·시간 등 실제 정보는
           절대 바꾸지 않는다 — 순서와 표현만 정리하는 것이다.
