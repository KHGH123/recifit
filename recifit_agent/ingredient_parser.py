...
_UNIT_RE = re.compile(
    ...
    # "고추가루 1스푼 (7g)"처럼 단위 뒤에 괄호로 환산량을 덧붙이는 표기가
    # 있다 — 이 괄호를 못 건너뛰면 앞의 unit 그룹까지 통째로 매치가
    # 실패해서 줄 전체가 "이름"으로 처리되고(단위 인식 실패), 검색 키워드도
    # "고추가루 1스푼 (7g)"라는 지저분한 문자열 그대로 나가 결과가 잘 안
    # 나온다. 괄호 내용 자체는 버리고 건너뛰기만 한다.
    r"(?:\([^)]*\))?\s*$",
    re.IGNORECASE,
)
